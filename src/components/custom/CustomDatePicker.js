...
export default function CustomDatePicker({ 
  ...
  // Gelecek 2 yıl için yıl listesi
  const getYears = () => {
  ...
  // Ay veya yıl değiştiğinde günü kontrol et ve gerekirse düzelt
  useEffect(() => {
    const maxDaysInMonth = getDaysInMonth(selectedMonth, selectedYear).length;
  ...
  const handleConfirm = () => {
    const dateObject = new Date(selectedYear, selectedMonth, selectedDay);
    const dateString = `${selectedDay} ${monthNames[selectedMonth]} ${selectedYear}`;
